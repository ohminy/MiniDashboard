...
void AsyncFSWebServer::begin(fs::FS* fs) {
	_fs = fs;
//	_fs->begin();
...
bool AsyncFSWebServer::load_config() {
	fs::File configFile = _fs->open(CONFIG_FILE, "r");
	if (!configFile) {
...
bool AsyncFSWebServer::save_config() {
	...
	fs::File configFile = _fs->open(CONFIG_FILE, "w");
	if (!configFile) {
...
bool AsyncFSWebServer::load_user_config(String name, String &value) {
	fs::File configFile = _fs->open(USER_CONFIG_FILE, "r");
	if (!configFile) {
		DEBUGLOG("Failed to open config file");
		return false;
	}
...
bool AsyncFSWebServer::loadHTTPAuth() {
	fs::File configFile = _fs->open(SECRET_FILE, "r");
	if (!configFile) {
...
void AsyncFSWebServer::configureWifiAP() {
	...
	//WiFi.disconnect();
	WiFi.mode(WIFI_AP);
	String APname = (String)_config.deviceName.c_str() + "_" + (String)ESP.getChipId();
	if (_httpAuth.auth) {
		...
		Serial.print(APname.c_str()); Serial.println("config (without Pass)");
	}
	if (CONNECTION_LED >= 0) {
...
void AsyncFSWebServer::configureWifi() {
	...
	Serial.print("Connecting to "); Serial.println(_config.ssid.c_str());
	...
	if (!_config.dhcp) {
		...
		WiFi.config(_config.ip, _config.gateway, _config.netmask, _config.dns);
          ...
          Serial.println("waitForConnectResult");
...
/*	ArduinoOTA.onEnd(std::bind([](fs::FS *fs) {
		fs->end();
		DEBUGLOG("\r\nEnd OTA\r\n");
	}, _fs));
...
bool AsyncFSWebServer::saveHTTPAuth() {
	...
	//TODO add AP data to html
	fs::File configFile = _fs->open(SECRET_FILE, "w");
	if (!configFile) {
